...
function LoadMultipleUsers(userCount){
    ...
    .then(mufa => {
        ...
        for(let i = 0; i < mufa.results.length; i++){
            let fn = '<div>' + mufa.results[i].name.first + '</div>'
            let ln = '<div>' + mufa.results[i].name.last + '</div>'
            let phone = '<div>' + mufa.results[i].phone + '</div>'
            let img = '<img src="' + mufa.results[i].picture.large + '">'
